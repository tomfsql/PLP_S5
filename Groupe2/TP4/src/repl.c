...
int main()
{
    ...
    while (continuer)
    {
        ...
        if(commande_trouvee == 0 && strlen(commande) > 0){
                    // On tente de transformer l'entrée en postfixé
                    TokenTab pf = postfix(commande);
                    
                    // On vérifie si le lexer n'a pas trouvé d'erreurs avant d'évaluer
                    int error_lex = 0;
                    ...
                    if(!error_lex) {
                        ...
                        // On affiche le résultat seulement s'il n'y a pas eu d'erreur de calcul
                        if(calcul_erreur == 0) {
                            printf("res : %f \n", res);
                        }
                    } else {
                        printf("Calcul annulé car l'expression contient des caractères invalides.\n");
                    }
                }
    }
    return 0;
